...
exports.getDeviceHistoryData = async (req, res) => {
  ...
  try {
    ...
    const data = await gpsService.fetchDeviceHistoryData(device_name, start_time, end_time);
...
async function insertOverhourEvent(stop, plateNo) {
  try {
      ...
      console.warn(`[SKIP] Tidak ditemukan vehicle_id untuk ${stop.source === 'gps_kit' ? stop.deviceName : (plateNo || 'N/A')}`);
      return false;
      }
    ...
    const platNomor = stop.source === 'gps_kit'
      ...
      platNomor,
        ...
        console.log(`[INSERTED] overhour event from ${stop.from} to ${stop.to} - Driver: ${driverInfo.name || 'N/A'}`);
...
async function insertOverspeedEvent(item, plateNo) {
  try {
      ...
      console.warn(`[SKIP] Tidak ditemukan vehicle_id untuk ${item.source === 'gps_kit' ? item.deviceName : (plateNo || 'N/A')}`);
      return false;
        }
    ...
    const platNomor = item.source === 'gps_kit'
      ...
      platNomor,
          ...
          console.log(`[INSERTED] overspeed event at ${item.datetime} - Driver: ${driverInfo.name || 'N/A'}`);
...
async function processGpsEventsBatch(items, plateNo, eventType) {
  ...
  console.log(`[BATCH] Processing ${items.length} ${eventType} events using bulk insert...`);
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        results.failed++;
        console.error(`[ERROR] Failed to prepare ${eventType} event:`, error.message);
      }
    }
    ...
    if (bulkData.length > 0) {
      ...
      console.log(`[BULK INSERT] Successfully inserted ${bulkData.length} ${eventType} events`);
    }

  } catch (error) {
    console.error(`[BULK ERROR] Failed to insert ${eventType} events:`, error.message);
    results.failed = results.total;
    results.success = 0;
  }

  console.log(`[BATCH] ${eventType} processing completed: ${results.success} success, ${results.failed} failed`);
  return results;
}
...
function logGpsProcessingSummary(identifier, summary) {
  ...
  console.log(`  Overhour: ${summary.events_processed.overhour.success}/${summary.events_processed.overhour.total} (${summary.events_processed.overhour.failed} failed)`);
  console.log(`  Overspeed: ${summary.events_processed.overspeed.success}/${summary.events_processed.overspeed.total} (${summary.events_processed.overspeed.failed} failed)`);
  console.log('='.repeat(80));
}
...
exports.getCombinedHistory = async (req, res) => {
  const { identifier, start_time, end_time, process_events = 'true' } = req.query;
  ...
  const isPlateNo = /^[A-Z]\s+\d+\s+[A-Z]+$/.test(identifier.replace(/\s+/g, ' '));
  ...
  if (isDeviceName) {
    ...
    plateNo = null;
    console.log(`[INFO] Identifier "${identifier}" detected as TotalKilat device name`);
  } else if (isPlateNo) {
    // This is a plate number for Margono
    plateNo = identifier;
    device_name = null;
    console.log(`[INFO] Identifier "${identifier}" detected as Margono plate number`);
  } else {
    // Try to determine by checking both systems
    // First try as device name, then as plate number
    device_name = identifier;
    plateNo = identifier;
    console.log(`[INFO] Identifier "${identifier}" will be tried on both systems`);
  }
  ...
  try {
    ...
    if (device_name) {
      try {
        console.log(`[DEBUG] Fetching TotalKilat data for device: ${device_name}`);
        ...
        console.log(`[DEBUG] Raw GPS response:`, JSON.stringify(gpsRaw, null, 2));
        ...
        gpsData = gpsData.map(item => ({
          ...
          odometer_km: (typeof item.mileage !== 'undefined' && item.mileage !== null)
            ? Number(item.mileage)
            : (typeof item.odometer_km !== 'undefined' ? Number(item.odometer_km) : undefined)
        }));
        ...
        if (gpsData.length > 0) {
          ...
          console.log(`[DEBUG] First GPS item datetime type:`, typeof gpsData[0].datetime);
        }
      } catch (gpsError) {
        console.warn(`[GPS ERROR] Failed to fetch TotalKilat data for device ${device_name}:`, gpsError.message);
        gpsData = [];
      }
    }
      ...
      try {
        ...
        margonoData = Array.isArray(margonoRaw?.Data)
          ? margonoRaw.Data.map(item => ({
              ...
              geoName: '',
              source: 'margono',
              odometer_km: (typeof item.Miles !== 'undefined' && item.Miles !== null)
                ? Math.round(Number(item.Miles) / 1000)
                : undefined
            }))
          : [];
      } catch (margonoError) {
        console.warn(`[MARGONO ERROR] Failed to fetch Margono data for plate ${plateNo}:`, margonoError.message);
        margonoData = [];
      }
    }

    const combined = [...gpsData, ...margonoData];
    console.log(`[DEBUG] Before filtering - GPS data: ${gpsData.length}, Margono data: ${margonoData.length}`);
    console.log(`[DEBUG] After filtering - Combined data: ${combined.length}`);
    if (combined.length === 0 && (gpsData.length > 0 || margonoData.length > 0)) {
      console.log(`[DEBUG] Filtering removed all items. Sample items before filter:`, 
    ...
    if (process_events === 'true') {
      const processEventsAsync = async () => {
        try {
          ...
          const overhourResults = await processGpsEventsBatch(longStops, device_name || plateNo, 'overhour');
          
          // Process overspeed events in batch
          const overspeedResults = await processGpsEventsBatch(overspeedItems, device_name || plateNo, 'overspeed');
          
          // Log processing results
          console.log(`[ASYNC] Events processing completed - Overhour: ${overhourResults.success}/${overhourResults.total}, Overspeed: ${overspeedResults.success}/${overspeedResults.total}`);
        } catch (error) {
          console.error('[ASYNC ERROR] Events processing failed:', error.message);
        }
      };
    ...
    const summary = {
      ...
      events_processing: process_events === 'true' ? 'async_background' : 'disabled',
      events_detected: {
...
exports.getCombinedLastPosition = async (req, res) => {
  try {
    ...
    if (!forceRefresh) {
      ...
      if (cachedData && !isCacheStale(cacheKey, 3)) { // 3 minutes max age for combined data
        return res.status(200).json({
          success: true,
          status: 200,
          ...
          data: cachedData,
          cached: true,
          cache_age: Math.round((Date.now() - positionCache.getTtl(cacheKey)) / 1000)
        });
      }
    }
    ...
    try {
      const totalKilatData = await withTimeout(gpsService.fetchLatestVehiclePosition(), 15000);
    ...
    const margonoPromises = nomorPlatList.map(async (plateNo) => {
      try {
        const response = await withTimeout(gpsService.lastPositionMargono(plateNo), 8000);
    ...
    const validMargonoResults = margonoResults.filter(result => result !== null);
...
exports.getOdometerSummary = async (req, res) => {
  try {
    ...
    const isDateOnly = (v) => typeof v === 'string' && v.length === 10 && /\d{4}-\d{2}-\d{2}/.test(v);
    ...
    const withOdo = combined.filter(r => Number.isFinite(Number(r.odometer_km)));
    ...
    if (withOdo.length > 0) {
      ...
      overall = {
        ...
        distance_km: Math.max(0, Number(end.odometer_km) - Number(start.odometer_km))
      };
    }
    ...
    const per_day = Object.entries(perDayMap).sort(([a], [b]) => a.localeCompare(b)).map(([date, arr]) => {
      const first = arr[0];
      ...
      return {
        ...
        distance_km: Math.max(0, Number(last.odometer_km) - Number(first.odometer_km))
      };
    });
  ...
  try {
    ...
    const cacheInfo = {
      ...
      key_details: cacheKeys.map(key => ({
        ...
        age_seconds: Math.round((Date.now() - positionCache.getTtl(key)) / 1000),
...
exports.getGpsEventLogs = async (req, res) => {
  ...
  const { page = 1, limit = 50, start_date, end_date, search, status } = req.query;
  ...
  const buildDateUTC = (dateStr, timeStr) => new Date(`${dateStr}T${timeStr}Z`);
  ...
  if (startInstant && endInstant) {
    ...
    // Get total count using Sequelize
    const totalRecords = await models.gps_event_logs.count({
    ...
    // Get paginated data using Sequelize, force raw result for time fields
    const rows = await models.gps_event_logs.findAll({
      where: whereConditions,
    ...
    // Proses data: tambahkan field alamat, tampilkan waktu apa adanya dari database
    const processedRows = await Promise.all(rows.map(async (item) => {
      // Use address stored in DB (column `address`) as `alamat`
      const alamat = (item.address || item.alamat) ? (item.address || item.alamat) : null;
      ...
      if (Object.prototype.hasOwnProperty.call(obj, 'address')) delete obj.address;
      obj.alamat = alamat;
    ...
    if (!processedRows || processedRows.length === 0) {
      return res.status(200).json({
        success: true,
        status: 200,
        message: start_date || end_date || search || status !== undefined ? 
        ...
        filters: {
          ...
          search: search || null,
          status: (status !== undefined && status !== null && String(status).trim() !== '') ? parseInt(status) : null
        },
        data: [],
      });
    }
    ...
    if (error.code === 'ETIMEDOUT') {
      return res.status(408).json({
        success: false,
        status: 408,
        message: 'Database connection timeout. Please try again with a smaller limit.',
...
exports.getGpsHistoryForKafka = async (req, res) => {
  ...
  try {
    ...
    const result = await this.getCombinedHistoryLogic(identifier, start_time, end_time, process_events);
 ...
 */
exports.getCombinedHistoryLogic = async (identifier, start_time, end_time, process_events = 'true') => {
  // Determine if identifier is a device_name (TotalKilat) or plateNo (Margono)
  const isDeviceName = /^\d+$/.test(identifier);
  const isPlateNo = /^[A-Z]\s+\d+\s+[A-Z]+$/.test(identifier.replace(/\s+/g, ' '));
  
  console.log(`[KAFKA DEBUG] Identifier: "${identifier}"`);
  console.log(`[KAFKA DEBUG] isDeviceName: ${isDeviceName}, isPlateNo: ${isPlateNo}`);
  ...
  if (isDeviceName) {
    device_name = identifier;
    plateNo = null;
    console.log(`[KAFKA INFO] Identifier "${identifier}" detected as TotalKilat device name`);
  } else if (isPlateNo) {
    plateNo = identifier;
    device_name = null;
    console.log(`[KAFKA INFO] Identifier "${identifier}" detected as Margono plate number`);
  } else {
    device_name = identifier;
    plateNo = identifier;
    console.log(`[KAFKA INFO] Identifier "${identifier}" will be tried on both systems`);
  }
  ...
  if (device_name) {
    try {
      console.log(`[KAFKA DEBUG] Fetching TotalKilat data for device: ${device_name}`);
      const gpsRaw = await gpsService.fetchDeviceHistoryData(device_name, start_time, end_time);
      const gpsFlattened = flatten(gpsRaw);
      ...
      console.warn(`[KAFKA GPS ERROR] Failed to fetch TotalKilat data for device ${device_name}:`, gpsError.message);
      gpsData = [];
    }
  }
  ...
  if (plateNo) {
    try {
      ...
      console.warn(`[KAFKA MARGONO ERROR] Failed to fetch Margono data for plate ${plateNo}:`, margonoError.message);
      margonoData = [];
    }
  }
  ...
  if (process_events === 'true') {
    const processEventsAsync = async () => {
      try {
        ...
        console.log(`[KAFKA ASYNC] Events processing completed - Overhour: ${overhourResults.success}/${overhourResults.total}, Overspeed: ${overspeedResults.success}/${overspeedResults.total}`);
      } catch (error) {
        console.error('[KAFKA ASYNC ERROR] Events processing failed:', error.message);
      }
    };
...
exports.kafkaCronJob = async () => {
  console.log(`[KAFKA CRON] Starting hourly GPS collection at ${new Date().toISOString()}`);
  ...
  try {
    ...
    console.log(`[KAFKA CRON] Processing data from ${startTimeStr} to ${endTimeStr}`);
    ...
    for (let i = 0; i < vehicles.length; i += batchSize) {
      ...
      console.log(`[KAFKA CRON] Processing batch ${Math.floor(i / batchSize) + 1} (${batch.length} vehicles)`);
      ...
      const batchPromises = batch.map(async (vehicle) => {
        try {
          ...
          console.error(`[KAFKA CRON ERROR] Failed to process vehicle ${vehicle.identifier}:`, error.message);
          return {
            identifier: vehicle.identifier,
            source: vehicle.source,
            totalRecords: 0,
    ...
    console.error('[KAFKA CRON ERROR] Hourly GPS collection failed:', error.message);
  }
};
...
async function getAllActiveVehiclesForKafka() {
  try {
    ...
    console.log(`[KAFKA CRON] Found ${vehicles.length} active vehicles with GPS (${gpsKitVehicles.length} TotalKilat, ${margonoVehicles.length} Margono)`);
    return vehicles;
  } catch (error) {
    console.error('[KAFKA CRON ERROR] Failed to fetch vehicles:', error.message);
    return [];
  }
}
